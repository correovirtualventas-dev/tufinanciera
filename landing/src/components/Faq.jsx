  ...
  return (
    <section id="faq" className="py-20 bg-surface">
      <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
        <h2 className="text-3xl sm:text-4xl font-bold text-center mb-4">Preguntas Frecuentes</h2>
        <p className="text-white/60 text-center mb-12">Todo lo que necesitás saber</p>
        <div className="space-y-4">
          {faqs.map((faq, i) => (
            <div key={i} className="bg-surface-light rounded-xl border border-white/10 overflow-hidden">
              <button onClick={() => setOpenIndex(openIndex === i ? null : i)} className="w-full text-left px-6 py-4 flex justify-between items-center">
                ...
                <span className={`material-symbols-outlined transition-transform text-white/60 ${openIndex === i ? 'rotate-180' : ''}`}>expand_more</span>
              </button>
              {openIndex === i && (
                <div className="px-6 pb-4 text-white/60 text-sm animate-fade-in">{faq.a}</div>
              )}
            </div>
          ))}
        </div>
