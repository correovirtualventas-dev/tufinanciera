  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex gap-1 bg-surface-100 rounded-xl p-1 border border-white/10">
        {tabs.map(t => (
          <button key={t.key} onClick={() => setTab(t.key)} className={`px-4 py-2 rounded-lg text-sm transition-colors ${tab === t.key ? 'bg-primary-500 text-white' : 'text-white/60 hover:text-white'}`}>
            {t.label}
          </button>
        ))}
      </div>
      ...
      {tab === 'users' && (
        <div className="space-y-4">
          <div className="bg-surface-100 rounded-xl p-6 border border-white/5">
            <h3 className="text-white font-semibold mb-4">{editingUser ? 'Editar' : 'Nuevo'} Usuario</h3>
            <form onSubmit={e => { e.preventDefault(); createUserMutation.mutate(userForm); }} className="grid grid-cols-2 gap-4">
              ...
              <div className="col-span-2 flex gap-3">
                ...
                {editingUser && <button type="button" onClick={() => { setEditingUser(null); setUserForm({ name: '', email: '', password: '', role: 'ADMIN' }); }} className="text-white/60 hover:text-white">Cancelar</button>}
              </div>
            </form>
          </div>
                ...
                {(users || []).map((u: any) => (
                  <tr key={u.id} className="border-b border-white/5 hover:bg-white/5">
                    ...
                    <td className="py-3 px-4 text-right">
                      <button onClick={() => { setEditingUser(u); setUserForm({ name: u.name, email: u.email, password: '', role: u.role }); }} className="p-2 hover:bg-white/10 rounded-lg text-white/60 hover:text-primary-500"><Edit2 size={16} /></button>
                      <button onClick={() => deleteUserMutation.mutate(u.id)} className="p-2 hover:bg-white/10 rounded-lg text-white/60 hover:text-red-500"><Trash2 size={16} /></button>
                    </td>
                  </tr>
                ))}
              </tbody>
      ...
      {tab === 'settings' && (
        <div className="bg-surface-100 rounded-xl p-6 border border-white/5">
          <h3 className="text-white font-semibold mb-4">Configuración del Sistema</h3>
          <div className="space-y-4">
            {['initialCapital', 'alertDays', 'interestRate', 'maxInstallments', 'minAmount', 'maxAmount'].map(key => (
              <div key={key}>
                <label className="block text-sm text-white/60 mb-1 capitalize">{key.replace(/([A-Z])/g, ' $1')}</label>
                <input
                  type="text"
      ...
      {tab === 'backup' && (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div className="bg-surface-100 rounded-xl p-6 border border-white/5">
            <h3 className="text-white font-semibold mb-4">Backup</h3>
            <p className="text-white/60 mb-4">Descargar copia de seguridad de todos los datos</p>
          ...
          <div className="bg-surface-100 rounded-xl p-6 border border-white/5">
            <h3 className="text-white font-semibold mb-4">Restaurar</h3>
            <p className="text-white/60 mb-4">Seleccionar archivo JSON de backup para restaurar</p>
