...
export default function Contabilidad() {
  ...
  return (
    <div className="space-y-6">
      ...
      {summary && (
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          <div className="bg-surface-100 rounded-xl p-4 border border-white/5">
            <p className="text-white/60 text-sm">Capital Inicial</p>
            <p className="text-xl font-bold text-white">{formatCurrency(summary.initialCapital)}</p>
          </div>
          <div className="bg-surface-100 rounded-xl p-4 border border-white/5">
            <p className="text-white/60 text-sm">Total Prestado</p>
            ...
            <tbody>
              {(movements || []).slice(0, 50).map((m: any, i: number) => (
                <tr key={i} className="border-b border-white/5 hover:bg-white/5">
                  <td className="py-3 px-4 text-white/80 text-sm">{formatDate(m.date)}</td>
                  ...
                  <td className="py-3 px-4 text-white">{m.description}</td>
      ...
      {tab === 'gastos' && (
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          <div className="bg-surface-100 rounded-xl p-6 border border-white/5">
            <h3 className="text-white font-semibold mb-4">Registrar Gasto</h3>
        ...
        <div className="bg-surface-100 rounded-xl p-6 border border-white/5">
          <div className="flex gap-3 mb-4">
            <input type="text" value={categoryName} onChange={e => setCategoryName(e.target.value)} placeholder="Nueva categoría..."
              className="flex-1 bg-surface-400 border border-white/10 rounded-lg px-3 py-2 text-white" />
          ...
          <div className="space-y-2">
            {(categories || []).map((c: any) => (
              <div key={c.id} className="flex justify-between items-center bg-surface-400 rounded-lg p-3">
                ...
                <button onClick={() => deleteCatMutation.mutate(c.id)} className="text-white/60 hover:text-red-500">
                  <Trash2 size={16} />
                </button>
              </div>
            ))}
      ...
      {tab === 'resumen' && summary && (
        <div className="bg-surface-100 rounded-xl p-6 border border-white/5">
          <h3 className="text-white font-semibold mb-4">Configuración de Capital</h3>
          <p className="text-white/60">Capital inicial registrado: <span className="text-white font-bold">{formatCurrency(summary.initialCapital)}</span></p>
          <p className="text-white/60 mt-2">Puedes modificar el capital inicial desde <strong>Configuración</strong>.</p>
        </div>
      )}
    </div>
  );
