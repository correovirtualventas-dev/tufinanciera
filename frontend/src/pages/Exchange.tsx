    ...
    <div className="space-y-6">
      ...
      {summary && (
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          <div className="bg-surface-100 rounded-xl p-4 border border-white/5">
            <p className="text-white/60 text-sm">Compra ARS</p>
            <p className="text-xl font-bold text-white">{formatCurrency(summary.totalBoughtARS)}</p>
          </div>
          <div className="bg-surface-100 rounded-xl p-4 border border-white/5">
            <p className="text-white/60 text-sm">Compra USD</p>
            <p className="text-xl font-bold text-tertiary-500">${summary.totalBoughtUSD.toFixed(2)}</p>
          </div>
          <div className="bg-surface-100 rounded-xl p-4 border border-white/5">
            <p className="text-white/60 text-sm">Venta ARS</p>
            <p className="text-xl font-bold text-white">{formatCurrency(summary.totalSoldARS)}</p>
          </div>
          <div className="bg-surface-100 rounded-xl p-4 border border-white/5">
            <p className="text-white/60 text-sm">Venta USD</p>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="bg-surface-100 rounded-xl p-6 border border-white/5">
          <h3 className="text-white font-semibold mb-4">Nueva Operación</h3>
          <form onSubmit={e => { e.preventDefault(); createMutation.mutate(form); }} className="space-y-4">
            <div className="flex gap-3">
              <button type="button" onClick={() => setForm({ ...form, type: 'BUY' })}
                className={`flex-1 py-3 rounded-lg font-semibold transition-colors ${form.type === 'BUY' ? 'bg-secondary-500 text-black' : 'bg-surface-400 text-white/60'}`}>Compra USD</button>
              <button type="button" onClick={() => setForm({ ...form, type: 'SELL' })}
                className={`flex-1 py-3 rounded-lg font-semibold transition-colors ${form.type === 'SELL' ? 'bg-tertiary-500 text-white' : 'bg-surface-400 text-white/60'}`}>Venta USD</button>
            </div>
            <div>
              <label className="block text-sm text-white/60 mb-1">Tipo de Cambio</label>
            ...
            <div>
              <label className="block text-sm text-white/60 mb-1">Monto ARS</label>
              <input type="number" value={form.amountARS} onChange={e => updateAmounts(form.type, 'amountARS', Number(e.target.value))}
                className="w-full bg-surface-400 border border-white/10 rounded-lg px-3 py-2 text-white" />
            </div>
            <div>
              <label className="block text-sm text-white/60 mb-1">Monto USD</label>
              ...
              <tbody>
                {(operations || []).map((op: any) => (
                  <tr key={op.id} className="border-b border-white/5 hover:bg-white/5">
                    ...
                    <td className="py-3 px-4 text-right text-white">{formatCurrency(op.amountARS)}</td>
                    <td className="py-3 px-4 text-right text-white">${op.amountUSD.toFixed(2)}</td>
                    <td className="py-3 px-4 text-right text-white/80">${op.rate.toFixed(2)}</td>
                    <td className="py-3 px-4 text-right">
                      <button onClick={() => deleteMutation.mutate(op.id)} className="p-2 hover:bg-white/10 rounded-lg text-white/60 hover:text-red-500">
                        <Trash2 size={16} />
                      </button>
                    </td>
                  </tr>
