...
export default function InvestorDetail() {
  ...
  if (!summary) return <div className="text-slate-500 mt-20 text-center">Inversor no encontrado</div>;
  ...
  return (
    <div className="space-y-6">
      ...
      <p className="text-slate-500">TNA: {investor.tna}% | Moneda: {investor.currency}</p>

      <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
        <div className="bg-surface-100 rounded-xl p-4 border border-slate-100">
          <p className="text-slate-500 text-sm">Capital Base</p>
          <p className="text-xl font-bold text-slate-900">{formatCurrency(capitalBase)}</p>
        </div>
        <div className="bg-surface-100 rounded-xl p-4 border border-slate-100">
          <p className="text-slate-500 text-sm">Acreditado</p>
          <p className="text-xl font-bold text-secondary-500">{formatCurrency(totalAccrued)}</p>
        </div>
        <div className="bg-surface-100 rounded-xl p-4 border border-slate-100">
          <p className="text-slate-500 text-sm">Pagado</p>
          <p className="text-xl font-bold text-red-500">{formatCurrency(totalPaid)}</p>
        </div>
        <div className="bg-surface-100 rounded-xl p-4 border border-slate-100">
          <p className="text-slate-500 text-sm">Disponible</p>
          <p className="text-xl font-bold text-tertiary-500">{formatCurrency(availableBalance)}</p>
        </div>
        <div className="bg-surface-100 rounded-xl p-4 border border-slate-100">
          <p className="text-slate-500 text-sm">Diario</p>
      ...
      {tab === 'movements' && (
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          <div className="bg-surface-100 rounded-xl p-6 border border-slate-100">
            <h3 className="text-slate-900 font-semibold mb-4">Nuevo Movimiento</h3>
            <form onSubmit={e => { e.preventDefault(); movementMutation.mutate(movementForm); }} className="space-y-4">
              ...
              <input type="number" step="0.01" value={movementForm.amount} onChange={e => setMovementForm({ ...movementForm, amount: Number(e.target.value) })} placeholder="Monto" required className="w-full bg-surface-400 border border-slate-200 rounded-lg px-3 py-2 text-slate-900" />
              <input type="date" value={movementForm.date} onChange={e => setMovementForm({ ...movementForm, date: e.target.value })} className="w-full bg-surface-400 border border-slate-200 rounded-lg px-3 py-2 text-slate-900" />
              <input value={movementForm.notes} onChange={e => setMovementForm({ ...movementForm, notes: e.target.value })} placeholder="Notas" className="w-full bg-surface-400 border border-slate-200 rounded-lg px-3 py-2 text-slate-900" />
      ...
      {tab === 'accruals' && (
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          <div className="bg-surface-100 rounded-xl p-6 border border-slate-100">
            <h3 className="text-slate-900 font-semibold mb-4">Recalcular Acreditaciones</h3>
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <div><label className="block text-sm text-slate-500 mb-1">Desde</label><input type="date" value={accrualRange.startDate} onChange={e => setAccrualRange({ ...accrualRange, startDate: e.target.value })} className="w-full bg-surface-400 border border-slate-200 rounded-lg px-3 py-2 text-slate-900" /></div>
                <div><label className="block text-sm text-slate-500 mb-1">Hasta</label><input type="date" value={accrualRange.endDate} onChange={e => setAccrualRange({ ...accrualRange, endDate: e.target.value })} className="w-full bg-surface-400 border border-slate-200 rounded-lg px-3 py-2 text-slate-900" /></div>
              </div>
              ...
              <tbody>
                {(accruals || []).map((a: any) => (
                  <tr key={a.id} className="border-b border-slate-100 hover:bg-slate-100">
                    <td className="py-2 px-4 text-slate-700">{a.date}</td>
                    <td className="py-2 px-4 text-right text-slate-900">{formatCurrency(a.capitalBase)}</td>
                    <td className="py-2 px-4 text-right text-slate-700">{a.tna}%</td>
                    <td className="py-2 px-4 text-right text-secondary-500">{formatCurrency(a.amount)}</td>
                    <td className="py-2 px-4 text-right"><button onClick={() => deleteAccrualMutation.mutate(a.id)} className="text-slate-500 hover:text-red-500"><Trash2 size={16} /></button></td>
                  </tr>
                ))}
              </tbody>
            </table>
      ...
      {tab === 'payouts' && (
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          <div className="bg-surface-100 rounded-xl p-6 border border-slate-100">
            <h3 className="text-slate-900 font-semibold mb-4">Nuevo Pago</h3>
            <p className="text-slate-500 text-sm mb-4">Disponible: {formatCurrency(availableBalance)}</p>
            <form onSubmit={e => { e.preventDefault(); payoutMutation.mutate(payoutForm); }} className="space-y-4">
              <input type="number" step="0.01" value={payoutForm.amount} onChange={e => setPayoutForm({ ...payoutForm, amount: Number(e.target.value) })} placeholder="Monto" required max={availableBalance} className="w-full bg-surface-400 border border-slate-200 rounded-lg px-3 py-2 text-slate-900" />
              <input type="date" value={payoutForm.date} onChange={e => setPayoutForm({ ...payoutForm, date: e.target.value })} className="w-full bg-surface-400 border border-slate-200 rounded-lg px-3 py-2 text-slate-900" />
              <input value={payoutForm.notes} onChange={e => setPayoutForm({ ...payoutForm, notes: e.target.value })} placeholder="Notas" className="w-full bg-surface-400 border border-slate-200 rounded-lg px-3 py-2 text-slate-900" />
              ...
              <tbody>
                {(payouts || []).map((p: any) => (
                  <tr key={p.id} className="border-b border-slate-100 hover:bg-slate-100">
                    <td className="py-2 px-4 text-right text-slate-900">{formatCurrency(p.amount)}</td>
                    <td className="py-2 px-4 text-right text-slate-500">{formatDate(p.date)}</td>
                    <td className="py-2 px-4 text-right"><button onClick={() => deletePayoutMutation.mutate(p.id)} className="text-slate-500 hover:text-red-500"><Trash2 size={16} /></button></td>
                  </tr>
                ))}
              </tbody>
            </table>
