      ...
      <div className="flex gap-1 bg-surface-100 rounded-xl p-1 border border-white/10">
        {tabs.map(t => (
          <button key={t.key} onClick={() => setTab(t.key)}
            className={`flex items-center gap-2 px-4 py-2 rounded-lg text-sm transition-colors ${tab === t.key ? 'bg-primary-500 text-white' : 'text-white/60 hover:text-white'}`}>
            ...
            <tbody>
              {(overdue || []).map((loan: any) => (
                <tr key={loan.id} className="border-b border-white/5 hover:bg-white/5">
                  <td className="py-3 px-4 text-white">{loan.client?.firstName} {loan.client?.lastName}</td>
                  ...
                  <td className="py-3 px-4 text-right text-white">{formatCurrency(loan.totalAmount)}</td>
              ...
              {(!overdue || overdue.length === 0) && (
                <tr><td colSpan={5} className="py-8 text-center text-white/40">Sin préstamos vencidos</td></tr>
              )}
            </tbody>
          </table>
        </div>
            ...
            <tbody>
              {(upcoming || []).map((item: any, i: number) => (
                <tr key={i} className="border-b border-white/5 hover:bg-white/5">
                  <td className="py-3 px-4 text-white">{item.client?.firstName} {item.client?.lastName}</td>
                  <td className="py-3 px-4 text-center text-white">#{item.installment}</td>
                  <td className="py-3 px-4 text-right text-white">{formatCurrency(item.amount)}</td>
                  <td className="py-3 px-4 text-right text-white/80">{formatDate(item.dueDate)}</td>
                  <td className="py-3 px-4 text-center">
            ...
            <tbody>
              {(pendingCollection || []).map((item: any) => (
                <tr key={item.id} className="border-b border-white/5 hover:bg-white/5">
                  <td className="py-3 px-4 text-white">{item.client?.firstName} {item.client?.lastName}</td>
                  <td className="py-3 px-4 text-right text-white">{formatCurrency(item.totalAmount)}</td>
